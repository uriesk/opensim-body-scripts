...
string getBase64AlphaString()
{
    ...
    s_base64alpha = base64And(s_base64alpha, gs_filterPrims);
...
default
{
    ...
    listen(integer channe, string name, key id, string message)
    {
        ...
        if (ownerOfSpeaker != ownerOfThisObject)
        {
            ...
            //to still catch those events, we do not return if id is llGetOwnerKey(id)
            //AND if speaker is not an avatar.
            //Could this be a potential security issue?!
            if (id != ownerOfSpeaker || llGetAgentSize(ownerOfSpeaker) != ZERO_VECTOR)
            {
                return;
            }
        }
        ...
        if (channe == gi_SkinChannel)
        {
            ...
            if (s_cmd == "upper")
            {
                ...
                setTextureBase64(base64And(gs_UpperParts, gs_filterPrims), s_uuid);
            }
            else if (s_cmd == "lower")
            {
                ...
                setTextureBase64(base64Or(base64And(gs_LowerParts, gs_filterPrims), s_base64feet), s_uuid);
            }
            else if (s_cmd == "neck")
            {
                if (gl_neck == []) return;
                setTextureList(gl_neck, s_uuid);
            }
            else if (s_cmd == "athena-handnails")
            {
                if (gl_handNails == []) return;
                setTextureList(gl_handNails, s_uuid);
            }
            else if (s_cmd == "athena-feetnails")
            {
